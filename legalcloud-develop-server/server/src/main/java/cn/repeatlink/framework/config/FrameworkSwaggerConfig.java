...
public class FrameworkSwaggerConfig {
	...
	public Docket framework_api() {
		...
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(apiInfo()).groupName("框架").select()
				// 为当前包路径
				.apis(RequestHandlerSelectors.basePackage("cn.repeatlink.framework")).paths(PathSelectors.any()).build()
