...
import cn.repeatlink.module.law.vo.cases.ReqCaseEstateVo;
...
@RestController("lawEstateCaseController")
@RequestMapping(Define.APP_URL_PREFIX + "/estatecase")
@Api(value="不动产案件管理", tags = "不动产案件")
@ApiSort(3)
public class EstateCaseController extends BaseLawController {
	...
	@ApiOperationSupport(order = 1, ignoreParameters = {"vo.datas", "vo.size", "vo.search_vo.estate_list", "vo.search_vo.case_id", "vo.search_vo.fetch_api"
			, "vo.search_vo.start_date", "vo.search_vo.step_buyer_input", "vo.search_vo.step_deal_finish", "vo.search_vo.step_reg_finish"
	...
	@ApiOperationSupport(order = 2, ignoreParameters = {"status"})
...
//	}
	...
	@ApiOperationSupport(order = 130, ignoreParameters = { })
	@ApiOperation(value="更新案件物件",notes = "更新案件物件")
	@ApiImplicitParam(required = true, dataType = "String", paramType = "path", name = "case_id", value = "案件ID")
	@PostMapping("/{case_id}/estate")
	public AjaxResult<EstatecaseDetailVo> updateCaseEstate(@PathVariable("case_id")String caseId, @RequestBody ReqCaseEstateVo vo) {
		// 操作权限检查
		this.checkCaseOpera(super.judicialUserId(), caseId);
		this.estateCaseService.updateCaseEstates(super.judicialUserId(), caseId, vo.getEstate_list());
		return AjaxResult.success();
	}
