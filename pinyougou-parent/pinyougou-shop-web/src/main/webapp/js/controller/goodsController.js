...
app.controller('goodsController' ,function($scope,$controller ,$location  ,goodsService,uploadService,itemCatService,typeTemplateService){	
    ...
    /*解释一下：第一个用的是平常自定义一个方法，因为他的父id为0，加载页面的时候初始化一下这个方法就ok，
    后两个利用的是angular的监控变量的方法，记住就行了，目的是监控定义的变量，然后触发这个方法
    实现动态效果，平常我们都用点击事件的方法触动。原理都一样。实现的方法不一样而已。*/
	//查询一级商品分类列表
	...
	/*利用AngularJS自身封装的一个“ w a t c h ” 对 象 来 实 现 动 态 监 听
	（ 下 拉 框 的 动 态 级 联 操 作 ） 。 watch”对象来实现动态监听（下拉框的动态级联操作）。
	watch”对象来实现动态监听（下拉框的动态级联操作）。
